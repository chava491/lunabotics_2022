...
class diggingWrapperROS():
    ...
    def callback_main(self, msg):
        ...
        if self.opcode >= 7 and self.opcode <= 20:
            #AUGER (DON'T CHANGE SPEED)
            if self.opcode == 7:
                self.digging.auger_dig(12)
            if self.opcode == 8:
                self.digging.auger_undig(12)
            if self.opcode == 9:
                self.digging.auger_stop()
            if self.opcode == 10:
    ...
    def stop(self):
        ...
        self.digging.dig_disengage_auger()
        self.digging.dig_disengage_pitch()

    def engage(self):
        self.digging.dig_engage_depth()
        self.digging.dig_engage_auger()
